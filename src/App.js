...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  const [isUploading, setUpdating] = useState(false);
...
// 3. Upload fact to database and receive the new fact object
setUpdating(true);
  ...
  return (
    <aside>
      <ul>
        ...
        {CATEGORIES.map((cat) => (
          <li key={cat.name} className="category" onClick={() => setCurrentCategory(cat.name)}>
            <button
